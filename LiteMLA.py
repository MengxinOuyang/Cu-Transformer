...
# Handle PyTorch autocast API change (compatible with both 1.x and 2.x)
try:
    from torch.amp import autocast as _autocast_cls
    def _autocast_decorator(fn):
        return _autocast_cls('cuda', enabled=False)(fn)
except (ImportError, TypeError):
    from torch.cuda.amp import autocast as _autocast_cls
    def _autocast_decorator(fn):
        return _autocast_cls(enabled=False)(fn)
